...
### Решение: в YOLO разрешить browser_evaluate / run_code_unsafe

**Контекст**: первая итерация YOLO (запись выше) гейтила
`browser_evaluate` и `browser_run_code_unsafe` даже в режиме «без
вопросов». Логика была: «произвольный JS — отдельный класс риска
независимо от mode». Через несколько часов работы пользователь
упёрся в первый же `browser_evaluate` при отладке Atlas-карточки в
Mini App (это основной debug-инструмент: forсe-click, dump DOM,
прочитать React state). С телефона ответить на approval-prompt он
не может.

**Что сделали**: убрали `_BROWSER_ALWAYS_ASK` gate из YOLO-ветки в
`make_can_use_tool`. Теперь в `/yolo on` оба browser-инструмента
auto-approve. В обычном режиме (`/yolo off`) — по-прежнему через
broker prompt.

**Обоснование, почему это безопасно**:
- `browser_evaluate` запускает JS в **page context** — sandbox
  браузера. Никакого FS-доступа, никаких системных вызовов, network
  только в пределах origin'а страницы. **Строго менее мощный** чем
  bash, который в YOLO уже разрешён без подтверждения. Гейтить
  sandbox-JS пока bash открыт — это театр безопасности, не
  безопасность.
- `browser_run_code_unsafe` запускает JS в **Node-процессе MCP
  сервера**. Этот процесс запущен на той же VPS, под тем же UID
  (root) что и dev-bot. Если Claude хочет выполнить произвольный
  код в этом процессе — он уже может через bash (`kill -SIGUSR1`,
  ptrace, `node -e ...`, что угодно). Никакого capability gain.
- Audit-trail сохраняется: каждый `yolo_auto_approved` логируется в
  `journalctl -u dev-bot` с tool name. Если что-то пойдёт не так —
  можно отследить.

**Что осталось гейтиться даже в YOLO**: только
`CATASTROPHIC_BASH_PATTERNS` (`rm -rf /`, dd, mkfs, форкбомба,
shutdown, iptables -F, userdel, passwd root, chmod -R 000 /).
Это последняя «pull-the-plug» проверка против brick-the-VPS
payloads — единственный реальный класс необратимого ущерба, который
никакой sandbox/Node-process гейт не накроет.

**Альтернативы**:
- Оставить как есть, ждать пока пользователь разлогинится в ноут.
  Отверг — пользователь явно попросил «дай ему больше доступов»,
  блокер реальный.
- Релаксировать только `browser_evaluate`, оставить
  `run_code_unsafe`. Отверг — последний всё равно эквивалентен по
  capability bash'у в той же VM. Гейтить только потому что в имени
  слово «unsafe» — карго-культ.
- Сделать `/yolo full` отдельным под-режимом для двух разных
  уровней. Отверг — overengineering. Если пользователь захочет
  обратно более строгий YOLO, лучше сделать `/safe-yolo` опцию
  тогда, под реальный use case.

**Lesson**: «дополнительный gate под существующим разрешением»
имеет смысл только если он закрывает дополнительный класс ущерба.
Если bash открыт, JS-в-странице ничего нового не открывает —
гейт == friction без security benefit.
