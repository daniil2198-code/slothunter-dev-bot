...
def _system_prompt() -> str:
    ...
    base = (
        ...
        "6. НЕ используй markdown-таблицы (``| col | col |``). Telegram "
        "HTML их не рендерит — они приходят сырым текстом, переносы "
        "ломают ячейки, читать невозможно. Вместо таблицы — bullet-list "
        "с key/value: ``• ruff: passed``, ``• mypy: 68 files clean``, "
        "``• pytest talon: 65/65``. Если данных много колонок — выводи "
        "построчно через ``**Header**: значение`` или приложи "
        "файлом через Write.\n"
        "7. Жирный/курсив/моноширно — пиши через MARKDOWN, не через "
        "сырой HTML. ``**жирный**`` (двойные звёздочки), ``*курсив*`` "
        "(одна), ``` `моноширно` ``` (бэктики). Конвертер на стороне "
        "бота преобразует это в Telegram HTML. Допустимо иногда "
        "написать ``<b>...</b>`` напрямую (whitelist пропустит), но "
        "MARKDOWN — идиоматичнее и его не сломает экранирование при "
        "вложенных кавычках/процитированных тегах."
    )
    if settings.playwright_mcp_enabled:
        base += (
            "\n\nBROWSER:\n"
